...
//		System.out.println(user1.getId());
	...
	public String selectedRooms(@RequestParam(name = "chooseroomId")int id) {
		...
		System.out.println("Id of room" + room.getRoomId());
		if(rUserId==0) {
			try {
			...
			user.getRooms().add(room);
				...
				return "redirect:/";
			} catch (ThisEmailisalreadyExist e) {
	...
	@RequestMapping("/check-mail/{id}")
		public void checkEmailId(@PathVariable String id) {
